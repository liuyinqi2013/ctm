...
namespace ctm
{
    ...
    void CBaseServer::OnHangUp(CConn* conn)
    {
    ...
    void CBaseServer::OnReady(CConn* conn)
    {
    ...
    void CBaseServer::OnError(CConn* conn)
    {
        CTM_DEBUG_LOG(m_log, "conn on error :[%s]", conn->ToString().c_str());
        conn->Close();
        OnClose(conn);
    }

    int CBaseServer::HandleIOEvent()
    {
        if (m_eventMonitor->WaitProc(10) != 0)
        {
    ...
    int CBaseServer::HandleReadyCConns()
    {
    ...
    CConn* CBaseServer::Listen(const string& ip, unsigned int port)
    {
    ...
    CConn* CBaseServer::Connect(const string& ip, unsigned int port)
    {
    ...
    int CBaseServer::Pipe(CConn* ConnArr[2])
    {
        int pipefd[2] = {0};
        int ret = pipe(pipefd);
        if (ret == -1)
    ...
    CConn* CBaseServer::CreateConn(int fd, int events, int status, bool listen, int family)
    {
